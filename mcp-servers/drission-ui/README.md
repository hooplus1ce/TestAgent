...
| 高级/调试 | `screenshot` `run_js` `mouse_trail` `download_by_browser` `listen_ws_start` `listen_ws_wait` `set_permission` `new_context` `switch_context` `list_contexts` |
...
| `DRISSION_UI_CAPS` | 启用的工具分组，逗号分隔（如 `core,vtable,filter`），或 `all` 启用全部 |

## 新功能：能力分组（Caps）

借鉴 Playwright MCP 设计，通过环境变量 `DRISSION_UI_CAPS` 按需启用工具，减少 LLM 上下文 token 消耗。
...
文件默认保存到 `HL_SHOT_DIR` 配置的目录（默认 `~/.drission-ui-shots`）。
