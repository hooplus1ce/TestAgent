...
### iframe 内元素坐标转换（工具栏 / 弹窗按钮）

所有 `page.mouse.click()` / `page.mouse.move()` 的坐标**必须是主页面视口坐标**（绝对坐标）。来自 iframe 内普通 DOM 元素（工具栏按钮、弹窗按钮等，非 VTable canvas）的坐标，需加 iframe 偏移：
...
**常见错误**：
- ❌ `page.mouse.click(elRect.left, elRect.top)` — 这是在 iframe 内取得的坐标，未加 iframe 偏移，点击位置偏移整个 iframe 的距离
- ❌ `page.mouse.click(vt.getCellRect(col,row).bounds.x1, ...)` — 未加 scroll 修正，滚动后偏移

**Scanner 产出（已算好视口坐标，直接使用）**：`scanColumns()` 输出的 `icons[].viewportX/Y` 已含全部偏移，无需再转换：
```javascript
...
VTable 的场景图（`scenegraph`）反映 canvas 上实际渲染的内容，是获取真实渲染状态的最可靠途径。`globalAABBBounds` 已反映 auto-fill 压缩和 `scrollToCell` 滚动后的真实渲染位置（与 §六「场景图坐标转换」一致）。
...
> 按列名批量取视觉文本的推荐方式是 `getColumnValuesByTitle()`，详见 `filter-validation.md` §三。本节是单单元格的底层写法。
...
// Step 2: 从左侧菜单栏重新进入目标模块（将「制造排产」替换为目标模块名）
