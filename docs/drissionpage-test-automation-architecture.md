...
5. 用例执行器只回放通过预检的显式 `automation_recipe`：每条用例必须有无歧义的业务断言，正则匹配有执行超时；普通 DOM 禁止 JS/坐标点击，持久化操作必须以布尔值声明 `destructive=true` 并提供始终执行的清理段。静态门禁覆盖 setup、steps、cleanup，`$ref` 解析后再次检查动态坐标、JS 和破坏性动作。执行前完成浏览器、会话和业务 iframe 就绪门；每条用例前后隔离页面状态，运行期可引用动态表格行，并记录实际数据、当次截图、性能和结构化失败原因。
6. 报告生成器输出需求场景、风险维度、页面/接口资产三类覆盖率，以及缺陷、P50/P95、当次证据和完整基线差异。执行、覆盖和报告快照使用完整产物脱敏，不受流证据默认集合上限截断；截图只从受信目录原子复制，报告链接经过协议校验和 URL 编码。基线比较显式区分回归、改善和中性变化；删除用例、状态降级、覆盖率下降及严格超过 20% 的耗时增长属于回归，当前结果不会自动覆盖历史基线。
...
- 不自动执行删除、提交、审批等破坏性操作；配方必须显式声明、由调用方确认，并提供确定性清理或状态恢复核验。动态 `$ref` 不能绕过运行期护栏。
- 不将用户名、密码、Cookie、Authorization、Token 或超大响应正文写入证据文件；用例、执行与报告产物落盘前同样脱敏。
- VTable 继续只通过 MCP façade 操作，技能和用例正文不暴露私有 DOM、JavaScript 或坐标细节。
- 回归基线不可被当前失败结果自动覆盖，避免将缺陷固化为预期；基线中的重复/空用例编号和未知状态会被拒绝。
- 已知缺陷复现必须提供 `defect_id`，不得反转业务 oracle 后计为正常通过；使用 `xfailed` 独立统计。若修复后的成功分支无法确定性恢复夹具，则只保留缺陷证据并从正式回归套件排除。
