...
```python
scan_table(kind="auto")      # → { ok, kind, columns/tables... }，VTable 时每列含 col/title/bodyBehavior/icons[]
```
...
当 `scan_table(kind="auto")` 返回 `{ok: false}` 时：
1. 排查根因：`.vtable` 元素不存在则等待 3s 重试 1 次
2. `screenshot()` 存档
3. 降级生成**仅低级展示类**用例（基于截图+scan_page_elements/dom_tree 可见列标题）
4. 备注标注：「VTable 实例未挂载，断言基于截图，建议人工复核」
5. 跳过所有需点击的交互用例（复选框/链接/排序/筛选）

## 三、行选择（复选框列）

Canvas VTable 的复选框列可通过 `click_table_cell(row=row, col=col, kind="vtable")` 直接点击。工具内部流程：
...
```python
# 点击第 N 行的复选框
click_table_cell(row=5, col=0, kind="vtable")   # col=0 为复选框列
```
...
⚠️ **`window.frameElement` 在 run_js 中可能 undefined**：算顶层视口坐标改用 `Array.from(window.parent.document.querySelectorAll('iframe')).find(f => f.contentWindow === window)`；或直接用 `click_table_cell` 工具（内部处理换算）。
...
|-------------|------|---------|
| 复选框 | `click_table_cell(row=row, col=col, kind="vtable")` 直接点击 | 行选中/取消 |
| 链接/按钮 | `click_table_cell(row=row, col=col, kind="vtable", double_click=True)` | 弹详情弹窗或跳转 |
...
### 5.2 点击后的三步检查
```python
# 1. click_table_cell 点击目标单元格
click_table_cell(row=row, col=col, kind="vtable")
...
```python
click_table_cell(row=0, col=col, icon_name="sort", kind="vtable")         # 排序
click_table_cell(row=0, col=col, icon_name="filter-icon", kind="vtable")  # 筛选
```
...
- `click_table_cell` / `hover_table_cell` 内部处理滚动和坐标换算
...
| 类型 | 判断条件 | 处理方式 |
|------|---------|---------|
| **交互弹窗** | `observe_start` → action → `observe_wait` 返回弹窗类型 | DFS 弹窗内部，关闭后继续 |
...
使用两段式观察：点击前安装监听，点击后读取信号并清理，避免漏掉短寿命 toast。
...
需要抓接口时：`observe_start(signals=[...,"network"], listen_targets="gateway")`，或使用 `listen_start` / `listen_wait` 做接口断言。
