...
6. 穷尽筛选字段 → 详见 **`references/filter-validation.md`**（含筛选模式切换/展开、输入类型检测、getColumnValuesByTitle 验证）
...
详见 `references/vtable-interaction.md`（挂载、列扫描、场景图 API、坐标转换、初始状态重置）和 `references/filter-validation.md`（getColumnValuesByTitle 验证）。
...
点击任意可交互元素后，必须**全量 DOM 扫描**检测弹窗，不能仅搜索 ant-design 组件——VTable 筛选弹窗是 `.vtable-filter-menu`，不是 `.ant-dropdown`。检测代码模板详见 **`references/modal-types.md`**（含弹窗/通知关闭规则）。
...
| VTable 挂载失败 | 按 `vtable-interaction.md` §二 降级流程：截图 + 仅生成低级展示类用例 |
