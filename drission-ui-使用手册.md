...
#### `cache_session()`
缓存当前 tab 的 SESSION/UCTOKEN/cookie_token 到服务器内存。连接后立即调用，供会话过期时恢复。
- **返回**：`{ok, cached:[...], missing:[...]}`
- **示例**：`cache_session()` → `{ok: true, cached: ["SESSION","UCTOKEN","cookie_token"]}`

#### `refresh_session()`
用缓存 cookie 经 CDP 注入并刷新页面，恢复过期会话。需先 `cache_session`。
...
| 类别 | 工具 |
|------|------|
| 连接/会话(5) | `connect` `cache_session` `refresh_session` `login_ocr` `check_session` |
